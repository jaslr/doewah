...
ssh root@${DROPLET_IP} "mkdir -p /root/doewah/releases"

# Copy APK to droplet
scp "$LOCAL_APK" root@${DROPLET_IP}:/root/doewah/releases/

# Update version.json on droplet
ssh root@${DROPLET_IP} "cat > /root/doewah/releases/version.json << EOF
{
  \"version\": \"${VERSION}\",
